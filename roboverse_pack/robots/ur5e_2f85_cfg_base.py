...
class Ur5E2F85Cfg(RobotCfg):
    ...
    # ee_body_name 保持为 wrist_3_link，但需要正确的 TCP 偏移
    ...
    # TCP 偏移修正：从 wrist_3_link 到 pinch site
    # flange: Y+0.1, 旋转180° 后，夹爪长度 0.1558m 变为 -Z 方向
    # 总偏移：X=0, Y=0.1, Z=-0.1558 (约 -0.16)
    curobo_tcp_rel_pos: tuple[float, float, float] = [0.0, 0.1, -0.1558]
    curobo_tcp_rel_rot: tuple[float, float, float] = [3.14159, 0.0, 0.0]  # 180° 绕 X 轴
