...
def extract_paths_from_mjcf(xml_file_path: str) -> list[str]:
    ...
    for rel in mesh_relpaths:
        candidate = mesh_basepath / rel
        if candidate.resolve().exists():
            mesh_abspaths.append(candidate)
        else:
            # Fallback: resolve relative to the XML file's directory instead of meshdir.
            # This handles cases like meshdir="assets" + file="../robotiq_2f85/assets/foo.stl"
            # where the ".." should escape beyond the meshdir base.
            fallback = path.parent / rel
            mesh_abspaths.append(fallback if fallback.resolve().exists() else candidate)    
