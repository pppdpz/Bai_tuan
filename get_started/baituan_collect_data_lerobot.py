...
def main():
    ...
    dataset = LeRobotDataset.create(
        ...
        features=features,
        use_videos=True,
        root="get_started/output/lerobot_dataset"
    )
    # -------------------------------------------------------------------
    ...
    for step in range(450):
        ...
        # 分离状态与记录 (只取前 6 个 UR5e 机械臂的主动关节，舍掉抓手多自由度关节数据)
        curr_q_np = curr_q.clone().cpu().numpy()[0][:6]
        current_state_np = np.concatenate([curr_q_np, [gripper_width]]).astype(np.float32)
        action_np = np.concatenate([target_joint_angles[0].cpu().numpy()[:6], [gripper_width]]).astype(np.float32)
    ...
    dataset.finalize()
    log.info("LeRobot Dataset saved successfully to get_started/output/lerobot_dataset")
    # ---------------------------------------------------------------
