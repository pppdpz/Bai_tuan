...
def get_trajectory_targets(step, device, robot_cfg, tcp_quat_xyzw):
    """
    ...
    Args:
        step: 当前步数 [0, 220)
    ...
    # waypoint_pos_pinch = [0.35, 0.0, 1.10]  # 中间安全点
    waypoint_pos_pinch = [0.3, 0.17, 1.10]  # 中间安全点

    # 接近位置（太阳能板上方，保持安全距离）
    # approach_pos_pinch = [0.35, 0.04, 1.28 + 0.15]  # = [0.15, 0.24, 1.43]
    approach_pos_pinch = [0.3, 0.24, 1.43]  # = [0.15, 0.24, 1.43]
    
    # 抓取位置（太阳能板边缘，折叠状态）
    # 目标：pinch 点对准 panel4_tip_L
    # grasp_pos_pinch = [0.35, 0.04, 1.28]
    grasp_pos_pinch = [0.3, 0.24, 1.28]
    
    # 展开位置（太阳能板完全展开后的位置）
    # 目标：拉动到展开状态的 panel4_tip_L
    # deploy_pos_pinch = [0.35, -0.50, 1.10]
    deploy_pos_pinch = [0.3, -0.30, 1.30]
    
    # 撤离位置（展开后向上移动，避免碰撞）
    # retract_pos_pinch = [0.35, -0.50, 1.10 + 0.20]  # = [0.15, -0.30, 1.30]
    retract_pos_pinch = [0.3, -0.30, 1.30]  # = [0.15, -0.30, 1.30]
...
for step in range(220):
    ...
    ee_pos_target, ee_quat_target, gripper_width = get_trajectory_targets(
        step, device, robot, REAL_TCP_DOWN_QUAT_XYZW
    )
