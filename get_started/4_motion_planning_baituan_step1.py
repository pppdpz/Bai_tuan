...
def get_trajectory_targets(step, device):
    ...
    # ===== 关键位置定义 =====
    # 注意：这些坐标是基于实际测量的太阳能板位置
    
    # 初始位置（当前机械臂位置）
    initial_pos = [1.0, 0.234, 1.975]
    
    # 接近位置（太阳能板上方10cm）
    approach_pos = [0.15, 0.24, 1.38]
    
    # 抓取位置（太阳能板边缘，折叠状态）
    grasp_pos = [0.15, 0.24, 1.30]
    
    # 展开位置（太阳能板完全展开后的位置）
    deploy_pos = [0.15, -0.30, 1.10]
    
    # 撤离位置（展开后向上和向外移动）
    retract_pos = [0.15, -0.20, 1.35]
