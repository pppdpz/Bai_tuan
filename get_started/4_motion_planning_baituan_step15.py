...
if pinch_site_id >= 0 and wrist_body_id >= 0:
    ...
    pinch_quat_xyzw = R.from_matrix(pinch_xmat).as_quat()  # scipy [x,y,z,w]
    wrist_quat_xyzw = R.from_matrix(wrist_xmat).as_quat()
    ...
    log.info(f"初始 TCP(pinch) 姿态 (x,y,z,w): [{pinch_quat_xyzw[0]:.4f}, {pinch_quat_xyzw[1]:.4f}, {pinch_quat_xyzw[2]:.4f}, {pinch_quat_xyzw[3]:.4f}]")
    log.info(f"初始 wrist3 位置: [{wrist_pos[0]:.4f}, {wrist_pos[1]:.4f}, {wrist_pos[2]:.4f}]")
    log.info(f"初始 wrist3 姿态 (x,y,z,w): [{wrist_quat_xyzw[0]:.4f}, {wrist_quat_xyzw[1]:.4f}, {wrist_quat_xyzw[2]:.4f}, {wrist_quat_xyzw[3]:.4f}]")
    
    # 保存真实的 TCP 朝下姿态供后续使用
    REAL_TCP_DOWN_QUAT_XYZW = pinch_quat_xyzw.copy()
    log.info(f"\n✓ 已保存真实 TCP 姿态到 REAL_TCP_DOWN_QUAT_XYZW")
else:
    log.error("无法找到 pinch site 或 wrist_3_link body")
    REAL_TCP_DOWN_QUAT_XYZW = np.array([0.0, 1.0, 0.0, 0.0])  # 回退值
...
#     """
#     将世界坐标系中的位姿转换到机器人 base_link 坐标系
    
#     Args:
#         world_pos: 世界坐标系中的位置 [1, 3] (torch tensor)
#         world_quat: 世界坐标系中的姿态 [1, 4] (x,y,z,w, torch tensor) ← 注意格式变化
#         base_pos: base_link 在世界坐标系中的位置 [1, 3] (torch tensor)
#         base_quat: base_link 在世界坐标系中的姿态 [1, 4] (w,x,y,z, torch tensor)
    
#     Returns:
#         base_pos_target: base 坐标系中的位置 [1, 3] (torch tensor)
#         base_quat_target: base 坐标系中的姿态 [1, 4] (w,x,y,z, torch tensor) ← IK 需要的格式
#     """
#     # 转换为 numpy 进行计算
#     device = world_pos.device
#     world_pos_np = world_pos.detach().cpu().numpy().reshape(3)
#     world_quat_np = world_quat.detach().cpu().numpy().reshape(4)  # [x,y,z,w] ← 现在是这个格式
#     base_pos_np = base_pos.detach().cpu().numpy().reshape(3)
#     base_quat_np = base_quat.detach().cpu().numpy().reshape(4)    # [w,x,y,z] ← robot config 格式
    
#     # 构建世界到目标的变换矩阵
#     T_world_to_target = np.eye(4)
#     # world_quat 已经是 [x,y,z,w] 格式，直接使用
#     T_world_to_target[:3, :3] = R.from_quat(world_quat_np).as_matrix()
#     T_world_to_target[:3, 3] = world_pos_np
    
#     # 构建世界到 base 的变换矩阵
#     T_world_to_base = np.eye(4)
#     # base_quat 是 [w,x,y,z] 格式，需要转换为 [x,y,z,w]
#     T_world_to_base[:3, :3] = R.from_quat([
#         base_quat_np[1], base_quat_np[2], base_quat_np[3], base_quat_np[0]
#     ]).as_matrix()
#     T_world_to_base[:3, 3] = base_pos_np
...
#     base_quat_target_xyzw = R.from_matrix(T_base_to_target[:3, :3]).as_quat()  # [x,y,z,w]
    
#     # 转换为 torch tensor，输出格式 [w,x,y,z]（IK 求解器需要的格式）
#     base_pos_target = torch.tensor(base_pos_target_np, device=device, dtype=torch.float32).unsqueeze(0)
#     base_quat_target = torch.cat([
#         torch.tensor([base_quat_target_xyzw[3]], device=device, dtype=torch.float32),  # w
#         torch.tensor(base_quat_target_xyzw[0:3], device=device, dtype=torch.float32)   # x,y,z
#     ]).unsqueeze(0)  # 最终输出: [w,x,y,z] 给 IK 求解器
...
def world_pose_to_base_pose(world_pos, world_quat, base_pos, base_quat):
    """
    将世界坐标系中的位姿转换到 IK 使用的 base_link 坐标系。

    输入约定：
        world_quat: [x, y, z, w]
        base_quat:  [w, x, y, z]   (来自 robot config / 当前代码约定)

    输出约定（给 IK）：
        base_quat_target: [w, x, y, z]
    """
    device = world_pos.device

    # ---- to numpy ----
    world_pos_np = world_pos.detach().cpu().numpy().reshape(3)
    world_quat_np = world_quat.detach().cpu().numpy().reshape(4)   # [x,y,z,w]
    base_pos_np = base_pos.detach().cpu().numpy().reshape(3)
    ...
    # ---- T_world_base(mjcf ur5e_base) ----
    T_world_to_base = np.eye(4)
    T_world_to_base[:3, :3] = R.from_quat(
        [base_quat_np[1], base_quat_np[2], base_quat_np[3], base_quat_np[0]]
    ).as_matrix()  # convert [w,x,y,z] -> [x,y,z,w]
    T_world_to_base[:3, 3] = base_pos_np

    # 目标先转到 MJCF 的 ur5e_base 坐标系
    T_base_to_target = np.linalg.inv(T_world_to_base) @ T_world_to_target

    # ------------------------------------------------------------------
    # 关键补偿：MJCF ur5e_base -> URDF base_link 存在固定 Z 轴 180°偏置
    # URDF里可对应到 base_link -> base = Rz(-pi)，故 base -> base_link = Rz(+pi)
    # 这里把目标从 "base(=ur5e_base)" 表达转换成 "base_link" 表达再给 IK。
    # ------------------------------------------------------------------
    T_base_to_base_link = np.eye(4)
    T_base_to_base_link[:3, :3] = R.from_euler("z", np.pi).as_matrix()

    # T_base_link_target = inv(T_base_base_link) * T_base_target
    T_base_link_to_target = np.linalg.inv(T_base_to_base_link) @ T_base_to_target

    # ---- extract ----
    base_pos_target_np = T_base_link_to_target[:3, 3]
    base_quat_target_xyzw = R.from_matrix(T_base_link_to_target[:3, :3]).as_quat()  # [x,y,z,w]
...
def get_trajectory_targets(step, device, robot_cfg, tcp_quat_xyzw, panel_tip_world_pos=None):
    """
    ...
    Args:
        step: 当前步数 [0, 220)
        device: torch device
        robot_cfg: 机器人配置（保留但未使用）
        tcp_quat_xyzw: TCP 朝下的真实姿态 [x,y,z,w] (从 MuJoCo 读取)
    ...
    """

    # 当前 pinch 位置（初始状态）
    initial_pos_pinch = [0.2377, 0.3421, 1.2082]

    # 中间安全点（阶段划分保持不变）
    waypoint_pos_pinch = [0.3, 0.17, 1.10]

    # 默认回退目标（site 读取失败时使用）
    approach_pos_pinch = [0.3, 0.24, 1.43]
    grasp_pos_pinch = [0.3, 0.24, 1.28]
    deploy_pos_pinch = [0.3, -0.30, 1.30]
    retract_pos_pinch = [0.3, -0.30, 1.35]
    ...
    quat_tcp_down_xyzw = REAL_TCP_DOWN_QUAT_XYZW.tolist()  # 转换为列表
    ...
    # ===== 阶段2: 精确定位 (步数 50-70) =====
        ...
        # ee_quat = quat_down
        # ee_quat = quat_wrist3_xyzw
        ee_quat = tcp_quat_xyzw  # 使用真实的 TCP 姿态
    ...
    ee_quat_tcp = torch.tensor([ee_quat], device=device, dtype=torch.float32)  # [x,y,z,w] 格式
...
ur5e_base_body_id = mj.mj_name2id(_model, mj.mjtObj.mjOBJ_BODY, "ur5e_base")
if ur5e_base_body_id < 0:
    raise RuntimeError("未找到 body: ur5e_base")
...
for step in range(220):
    ...
    ee_pos_target, ee_quat_target, gripper_width = get_trajectory_targets(
        step, device, robot, REAL_TCP_DOWN_QUAT_XYZW, panel_tip_world_pos
    )
    ...
    base_pos_np = _data.xpos[ur5e_base_body_id].copy()
    base_xmat = _data.xmat[ur5e_base_body_id].reshape(3, 3).copy()
    base_quat_xyzw = R.from_matrix(base_xmat).as_quat()  # [x,y,z,w]

    robot_base_pos_world = torch.tensor(base_pos_np, device=device, dtype=torch.float32).unsqueeze(0)
    robot_base_quat_world = torch.tensor(
        [base_quat_xyzw[3], base_quat_xyzw[0], base_quat_xyzw[1], base_quat_xyzw[2]],  # [w,x,y,z]
        device=device,
        dtype=torch.float32,
    ).unsqueeze(0)


    # ===== 关键修复：将世界坐标系目标转换到 base 坐标系 =====
    # IK 求解器期望的是相对于 base_link 的目标位姿，而不是世界坐标系
    # 如果不做这个转换，机器人会移动到错误的位置
