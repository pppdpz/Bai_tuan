...
def get_trajectory_targets(step, device, robot_cfg, tcp_quat_wxyz, panel_tip_world_pos=None):
    ...
    # ===== 修正：所有目标点都相对于机器人 base 位置 [1.0, 0.0, 0.895] =====
    # 机器人 base 在世界坐标系的位置
    base_x, base_y, base_z = 1.0, 0.0, 0.895
    
    # 当前 pinch 位置（初始状态）- 相对于 base 的偏移
    initial_pos_pinch = [base_x - 0.76, base_y + 0.34, base_z + 0.31]  # 约 [0.24, 0.34, 1.21]

    # 中间安全点
    waypoint_pos_pinch = [base_x - 0.70, base_y + 0.17, base_z + 0.21]  # 约 [0.30, 0.17, 1.10]

    # 默认回退目标（site 读取失败时使用）
    approach_pos_pinch = [base_x - 0.70, base_y + 0.24, base_z + 0.54]  # 约 [0.30, 0.24, 1.43]
    grasp_pos_pinch = [base_x - 0.70, base_y + 0.24, base_z + 0.39]     # 约 [0.30, 0.24, 1.28]
    deploy_pos_pinch = [base_x - 0.70, base_y - 0.30, base_z + 0.41]    # 约 [0.30, -0.30, 1.30]
    retract_pos_pinch = [base_x - 0.70, base_y - 0.30, base_z + 0.46]   # 约 [0.30, -0.30, 1.35]
