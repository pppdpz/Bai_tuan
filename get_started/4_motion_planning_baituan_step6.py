...
for step in range(220):
    ...
    # ===== 步骤3: 在阶段内部使用插值（可选，提供更平滑的运动）=====
    # 如果需要在阶段内部也进行平滑插值，可以添加以下代码：
    # else:
    #     # 在阶段内部，可以选择：
    #     # 方案A: 保持目标不变（使用上一次IK求解的结果）
    #     # 方案B: 每步都重新求解IK（当前方案）
    #     # 方案C: 在阶段内部进行关节空间插值
    #     pass
    ...
    if target_joint_angles is not None:
        ...
        actions = ik_solver.compose_joint_action(
            target_joint_angles,  # 使用阶段切换时求解的目标
            gripper_widths, 
            curr_q, 
            return_dict=True
        )
    else:
        # 如果还没有目标角度（第一步之前），使用当前角度
        reorder_idx = handler.get_joint_reindex(robot.name)
        inv_idx = [reorder_idx.index(i) for i in range(len(reorder_idx))]
        curr_q = obs.robots[robot.name].joint_pos[:, inv_idx]
        actions = ik_solver.compose_joint_action(curr_q, gripper_widths, curr_q, return_dict=True)
