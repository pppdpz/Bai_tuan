...
def get_trajectory_targets(step, device):
    ...
    # ===== 坐标偏移常量（实际测量值）=====
    OFFSET_X = 0.0000  # wrist_3_link 到 pinch 的 X 轴偏移
    OFFSET_Y = 0.2558  # wrist_3_link 到 pinch 的 Y 轴偏移
    OFFSET_Z = 0.0000  # wrist_3_link 到 pinch 的 Z 轴偏移

    # 辅助函数：将期望的 pinch 位置转换为 wrist_3_link 位置
    def pinch_to_wrist(pinch_pos):
        """
        将 pinch 点位置转换为 wrist_3_link 位置（IK求解器需要的）
        
        Args:
            pinch_pos: [x, y, z] pinch 点的期望位置
        
        Returns:
            [x, y, z] wrist_3_link 的目标位置
        """
        return [
            pinch_pos[0] - OFFSET_X,  # X 轴：pinch_x - 0.0 = pinch_x
            pinch_pos[1] - OFFSET_Y,  # Y 轴：pinch_y - 0.2558
            pinch_pos[2] - OFFSET_Z   # Z 轴：pinch_z - 0.0 = pinch_z
        ]
    ...
    # 转换为 wrist_3_link 位置（IK求解器需要的）
    initial_pos = pinch_to_wrist(initial_pos_pinch)
    approach_pos = pinch_to_wrist(approach_pos_pinch)
    grasp_pos = pinch_to_wrist(grasp_pos_pinch)
    deploy_pos = pinch_to_wrist(deploy_pos_pinch)
    retract_pos = pinch_to_wrist(retract_pos_pinch)
