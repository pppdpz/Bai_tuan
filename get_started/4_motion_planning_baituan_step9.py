...
def tcp_pose_to_wrist3_pose(tcp_pos, tcp_quat, offset_pos, offset_quat):
    ...
    # if is_torch:
    #     wrist3_pos = torch.tensor(wrist3_pos_np, device=device, dtype=torch.float32).unsqueeze(0)
    #     # 转换为 [w,x,y,z] 格式以匹配 pyroki/jaxlie 约定
    #     wrist3_quat_xyzw = torch.tensor(wrist3_quat_np, device=device, dtype=torch.float32)
    #     wrist3_quat = torch.cat([wrist3_quat_xyzw[3:4], wrist3_quat_xyzw[0:3]]).unsqueeze(0)
...
def get_trajectory_targets(step, device, robot_cfg, tcp_quat_xyzw):
    ...
    # 明确使用 scipy 的 [x,y,z,w] 格式
    # quat_wrist3_xyzw = [0.5, -0.5, 0.5, -0.5]  # (x, y, z, w) - scipy 标准格式
    ...
    # ee_quat_tcp = torch.tensor([quat_wrist3_xyzw], device=device, dtype=torch.float32)  # [x,y,z,w] 格式
