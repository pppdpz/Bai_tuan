...
for step in range(300):
    ...
    if step in [0, 50, 70, 90, 120, 200, 220, 250]:
        # 临时应用求解结果到 MuJoCo 数据结构（不执行仿真）
        _temp_qpos = _data.qpos.copy()
        _data.qpos[:len(q_sol[0])] = q_sol[0].cpu().numpy()
        mj.mj_forward(_model, _data)
        ...
        log.info(f"\n{'='*60}")
        log.info(f"IK 收敛性验证 (Step {step})")
        log.info(f"{'='*60}")
        log.info(f"目标 TCP 位置: [{_target_pos[0]:.4f}, {_target_pos[1]:.4f}, {_target_pos[2]:.4f}]")
        log.info(f"IK 求解到达: [{_pinch_pos_reached[0]:.4f}, {_pinch_pos_reached[1]:.4f}, {_pinch_pos_reached[2]:.4f}]")
        log.info(f"位置误差: {pos_error*1000:.2f} mm")
        
        # 特别关注 Y 轴误差（这是抓取最外端的关键）
        y_error = abs(_pinch_pos_reached[1] - _target_pos[1])
        log.info(f"Y 轴误差: {y_error*1000:.2f} mm (目标Y={_target_pos[1]:.4f}, 实际Y={_pinch_pos_reached[1]:.4f})")
        
        if pos_error > 0.01:  # 10mm 阈值
            log.warning(f"⚠️  IK 求解未收敛到目标！误差 {pos_error*1000:.1f}mm 超过 10mm")
            if y_error > 0.05:  # 50mm Y轴阈值
                log.error(f"❌ Y 轴误差过大！可能未到达最外端悬空点")
                log.error(f"   根据 FK 分析：Y=0.24 是最外端，Y=0.42 是内侧铰链群")
                log.error(f"   当前到达 Y={_pinch_pos_reached[1]:.4f}，距离目标还差 {y_error*100:.1f}cm")
        else:
            log.info(f"✓ IK 收敛良好，误差在可接受范围内")
        
        log.info(f"{'='*60}\n")
        
        # 恢复原始 qpos
        _data.qpos[:] = _temp_qpos
        mj.mj_forward(_model, _data)
    # ===== 验证结束 =====
...
# ===== 新增：任务完成后的诊断报告 =====
log.info(f"\n{'='*80}")
log.info(f"任务完成 - IK 收敛性诊断报告")
log.info(f"{'='*80}")

# 分析第二关节角度变化
if joint_2_angles_record:
    joint_2_values = [r['joint_2_deg'] for r in joint_2_angles_record]
    log.info(f"第二关节角度范围: {min(joint_2_values):.1f}° ~ {max(joint_2_values):.1f}°")
    
    # 检查关键阶段的角度
    key_steps = [50, 70, 90, 120]
    for s in key_steps:
        if s < len(joint_2_angles_record):
            log.info(f"  Step {s}: {joint_2_angles_record[s]['joint_2_deg']:.1f}°")

log.info(f"\n关键提示：")
log.info(f"1. 根据 FK 分析，Y=0.24 是最外端悬空点（panel_4_L 的舌头尖端）")
log.info(f"2. Y=0.42 是内侧铰链群（panel 1-3 折叠区）")
log.info(f"3. Y=0.52 是连接根部，Y=0.60 是卫星主体")
log.info(f"4. 如果 IK 验证显示 Y 轴误差 >5cm，说明求解器未到达最外端")
log.info(f"5. 可能的解决方案：")
log.info(f"   - 调整 IK 求解器的迭代次数和容差")
log.info(f"   - 检查关节限位是否阻止到达目标")
log.info(f"   - 使用更好的初始猜测（从当前位置逐步逼近）")
log.info(f"{'='*80}\n")
